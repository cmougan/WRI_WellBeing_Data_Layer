# Original code gist : https://gist.github.com/marmakoide
# Changes to the gist are here : https://github.com/sunayana/laguerre-voronoi
import itertools
import numpy as np
...
def disc_uniform_pick(N: int) -> np.ndarray:
...
def get_power_triangulation_with_boundary(S, R, Sbdry):
    S_modified = np.concatenate((S, Sbdry), axis=0)
    R_modified = np.concatenate((R, np.zeros(len(Sbdry))), axis=0)
    tri_list, V = get_power_triangulation(S_modified, R_modified)
    return tri_list, V
        ...
        for u, v, w in ((a, b, c), (b, c, a), (c, a, b)):
            ...
            if len(edge_map[edge]) == 2:
                ...
                voronoi_cell_map[u].append(
                    ((edge_map[edge][0], np.Inf), (D, W, 0, np.Inf))
                )
                voronoi_cell_map[v].append(
                    ((np.Inf, edge_map[edge][0]), (D, -W, -np.Inf, 0))
                )
...
def get_voronoi_cells_with_bdry(S, V, tri_list, Sbdry):
    S_modified = np.concatenate((S, Sbdry), axis=0)
    voronoi_cells = get_voronoi_cells(S_modified, V, tri_list)
    # Drop all cells with infinite segments.
...
def display(S, R, tri_list, voronoi_cell_map):
    ...
    for Si, Ri in zip(S, R):
        ax.add_artist(
            plot.Circle(Si, Ri, fill=True, alpha=0.4, lw=0.0, color="#8080f0", zorder=1)
        )
